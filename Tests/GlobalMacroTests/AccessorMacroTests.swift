...
final class AccessorMacroTests: XCTestCase {
    let testMacros: [String: Macro.Type] = {
        #if canImport(GlobalMacroMacros)
        [
            "Accessor": GlobalAccessorMacro.self,
        ]
        #else
        []
        #endif
    ...
    func testMacro() throws {
        ...
        assertMacroExpansion(
            ...
            extension SomeExtension {
                @Accessor(\\.state) var state: GlobalState
            }
            """,
            expandedSource: """
            extension SomeExtension {
    ...
    func testMacroExplicitGetter() throws {
        ...
        assertMacroExpansion(
            """
            extension SomeExtension {
                @Accessor(\\.state, type: .getter) var state: GlobalState
            }
            """,
            expandedSource: """
            extension SomeExtension {
    ...
    func testMacroWithSetter() throws {
        ...
        assertMacroExpansion(
            """
            extension SomeExtension {
                @Accessor(\\.state, type: .getterAndSetter) var state: GlobalState
            }
            """,
            expandedSource: """
            extension SomeExtension {
    ...
    func testMacroIgnoresAssignedValue() throws {
        ...
        assertMacroExpansion(
            """
            extension SomeExtension {
                @Accessor(\\.state) var state: GlobalState = .unknown
            }
            """,
            expandedSource: """
            extension SomeExtension {
