...
/// The macro `Accessor` is used in extensions when you can't or don't want to use the `@Global`
/// property wrapper to access the global value. It will create all the code needed to access
/// the value stored in `GlobalValues`.
///
/// To use this macro in an extension like so:
/// ```swift
/// extension UIView {
///     @Accessor(\.globalStatus) var status: Status
/// }
/// ```
///
...
///   Default: `.getter`
@attached(accessor)
public macro Accessor<V, KP: KeyPath<GlobalValues, V>>(
    _ keyPath: KP,
